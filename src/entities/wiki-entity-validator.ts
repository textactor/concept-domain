...
const schema = {
    ...
    partialNames: Joi.array().items(Joi.string().min(2).max(200).trim().required()),
    partialNamesHashes: Joi.array().items(Joi.string().trim().min(16).max(40).required()),
    ...
    types: Joi.array().items(Joi.string().trim().min(2).max(50).required()).max(50),
    countryCodes: Joi.array().items(Joi.string().regex(/^[a-z]{2}$/).required()),
    rank: Joi.number().integer().min(1),
    categories: Joi.array().items(Joi.string().trim().min(2).max(200).required()),
    data: Joi.object(),
    links: Joi.object(),
...
const updateSchema = Joi.object().keys({
    ...
    delete: Joi.array().valid('simpleName', 'specialName', 'abbr', 'description', 'about', 'type', 'types', 'data', 'categories'),
});
